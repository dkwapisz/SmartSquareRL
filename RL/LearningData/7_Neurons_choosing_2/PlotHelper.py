...
tested_parameter = ("800, 400", "1000, 200", "1000, 400", "1000, 800", "1000, 1000", "1200, 800", "1400, 1000")
episodes_limit = 2500
coins_in_level = 5
nr_of_workers = 6
...
for j in range(0, nr_of_workers, nr_of_plots):
    ...
    pylab.xticks([i for i in range(-0, 2600, 200)])
    pylab.yticks([i for i in range(-1400, 1100, 200)])
    pylab.ylim(-1400, 1100)
    ...
    for i in range(j, j+nr_of_plots, 1):
        plot = pylab.plot(multiple_x[i], multiple_y[i], linewidth=0.5, label="Neurony w warstwach ukrytych: " + str(tested_parameter[i]))
    pylab.legend(loc='lower right')

    pylab.savefig("wykresNeurony2.png")
    pylab.show()
