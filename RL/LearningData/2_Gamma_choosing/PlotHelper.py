...
tested_parameter = [0.80, 0.85, 0.9, 0.95, 0.99, 0.999]
episodes_limit = 2000
coins_in_level = 5
nr_of_workers = 6
...
for j in range(0, nr_of_workers, nr_of_plots):
    ...
    pylab.yticks([i for i in range(-1600, 200, 200)])
    pylab.ylim(-1600, 200)
    ...
    for i in range(j, j+nr_of_plots, 1):
        plot = pylab.plot(multiple_x[i], multiple_y[i], linewidth=0.5, label=r"$\gamma$ = " + str(tested_parameter[i]))
    pylab.legend(loc='lower right')

    pylab.savefig("wykresGamma.png")
    pylab.show()
