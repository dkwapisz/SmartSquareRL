...
tested_parameter = ("128, 16", "128, 32", "128, 64", "100, 20, 10", "128, 16, 16", "64, 16, 16", "64, 32, 16", "64, 32, 16, 8", "128, 32, 8, 8", "128, 32, 16, 8, 4")
episodes_limit = 2000
coins_in_level = 2
nr_of_workers = 10
...
for j in range(0, nr_of_workers, nr_of_plots):
    ...
    pylab.xticks([i for i in range(-0, 2100, 200)])
    pylab.yticks([i for i in range(-1200, 900, 200)])
    pylab.ylim(-1200, 900)
    ...
    for i in range(j, j+nr_of_plots, 1):
        plot = pylab.plot(multiple_x[i], multiple_y[i], linewidth=0.5, label="Neurony: " + tested_parameter[i])
    pylab.legend(loc='lower right')

    pylab.savefig("WykresGen7x7Proba2.png")
    pylab.show()
